...
public class ClientHandler extends AbstractClient {
	...
	public void EditQuestion(String newBody, String newQNumber, String originalId) {
        ...
        list.add("UPDATE `projecton`.`questions` SET `id` = '" + s + "', `questiontext` = '" + newBody + "', `questionnumber` = '" + newQNumber + "' WHERE (`id` = '"+originalId +"');");   
        //UPDATE `projecton`.`questions` SET `id` = '13', `questiontext` = '13', `questionnumber` = '13' WHERE (`id` = '01001');
        try {
            sendToServer((Object) list);
        } catch (IOException e) {
            e.printStackTrace();
